...
- Non-teammate corpses use filtered looting:
    ...
    - with `Pickup Gear` enabled, a working first primary, and an empty second primary, a usable found long gun becomes a registered vanilla support weapon; only compatible source magazines that fit fast access while preserving reload landing space join it, and this non-primary result uses `EPhraseTrigger.LootGeneric`
    ...
    - category filters from `Looting Settings` are checked before price for ordinary cargo: `Pickup Food`, `Pickup Meds`, `Pickup Valuables`, and `Pickup Gear`; corpse dogtags, missing-primary acquisition, and implemented true swaps bypass these category filters, but optional second-primary/holster weapon additions require `Pickup Gear`
    ...
    - with `Allow Gear Swapping` active, an equipped detachable-magazine primary may top off empty or partial compatible vest/pocket magazines independently of `Pickup Gear`; carried loose ammunition is used first, and Immersive/Realistic may use searched-source rounds after carried supply; top-off never unloads or replaces existing cartridges
...
- Normal cargo weapons are priced and moved as whole weapon trees. With `Allow Gear Swapping` active, a long gun can equip into an empty first-primary slot without min/max price or `Pickup Gear` when its loaded state and compatible fast-access magazines satisfy readiness. A working-primary follower may add a registered second-primary support weapon only when `Pickup Gear` is enabled and second primary is empty. For an empty magazine slot, the most-loaded compatible body/container magazine is inserted first as a staging transaction, then the same live loaded-weapon planner decides its destination. Once a primary or Pickup-Gear-authorized support plan is accepted, compatible loaded source magazines bypass normal loot filters only while they fit reload-safe fast access.
- Equipped-primary magazine top-off follows the same body-loot rule: `Allow Gear Swapping`, not `Pickup Gear`, owns the operation; carried loose ammunition is preferred, searched loose ammunition may be used in Immersive/Realistic, and no loaded cartridge is removed or replaced.
...
- Successful empty-primary weapon equip rebuilds the follower weapon-manager primary info and requests a main-hand switch so combat can use the new weapon.
- Once first and second primary are occupied, later long guns are ordinary filtered cargo and do not automatically take compatible magazines. An occupied holster applies the same rule to later pistols. With `Pickup Gear` enabled, other eligible weapons may still use an empty compatible slot before backpack/pocket fallback.
...
- Missing-primary acquisition and implemented true swaps bypass min/max price and the `Pickup Gear` category filter. Optional second-primary/holster weapon additions require `Pickup Gear`; ordinary gear cargo fallback respects both category and price.
