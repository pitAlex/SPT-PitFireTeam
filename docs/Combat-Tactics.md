...
- Launcher preparation owns one hold while hands switch and reload completes, with readiness/reload probes every `0.15s`. Its deadline is armed once: `3s` for an already-loaded launcher, or a `9s` total backstop when reloading is needed, while the shared reload attempt retains its existing `6s` limit. Partially loaded cylinder reloads are not ready until reloading ends and both selector and weapon manager are ready. Enemy loss, explicit commands, genuine preparation failure, and prepared survival actions can still release the wait; the `5s` firing-opportunity window starts only after readiness.
