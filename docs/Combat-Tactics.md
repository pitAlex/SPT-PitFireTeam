...
- Movement-facing uses one shared `12s` real-sighting window across walking/running pursuit, ordinary attack movement, tactical movement and run-to-cover arrival. The newest valid personal or real shared sighting supplies the remembered look point; sense-only timestamps and the unseen live transform cannot refresh that permission. An expired sighting may remain a search destination, but cannot stop a run as a close threat or force a backwards look at any range. Fresh/visible close threats retain the existing stop-and-face protection, and dedicated pressure/medical-retreat facing and fire gates remain unchanged. The run action selects and continues route-facing only while `Mover.Sprinting` and an active path are both present. Stopping sprint immediately releases that route-look commitment, allowing fresh threat-facing to take over; without a valid threat look, non-sprinting updates preserve their current look instead of forcing it toward the navigation path.
