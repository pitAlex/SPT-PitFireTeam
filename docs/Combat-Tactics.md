...
- if vanilla clears `GoalEnemy` between clustered contacts while recent incoming-fire awareness remains, keep the core combat layer active, select hidden lateral/backward cover from the existing per-frame candidate pool, and use committed `attackMovingWithSuppress`; do not fall into crouched post-combat linger in the open
- a memory-only contact cannot create an autonomous push mission or direct `runToEnemy` / `goToEnemy` advance, but it can use cautious cover/simple-search movement to investigate the reported position; if that search cannot produce movement, it uses the bounded stable no-cover hold contract instead of ending as `notInCover` every frame
...
- combat reload holds are only emitted after a reload actually starts; active reloads retain the hold, while rejected/completed reloads use a three-second retry cooldown so visible contact cannot create start/end churn. If EFT left a Rifleman holding a pistol, combat first selects a usable first/second primary; when neither has push-ready loaded ammo, the selected long gun enters the same bounded reload hold before pressure can resume.
...
- passive `coverHold` / `bossHold*` / no-push holds only end after the router has prepared a different actionable reaction. A recent hit while already covered prepares an immediate fight handoff first; if none exists, the follower retains cover under the same three-second retry. Failed visible or advance scans also stay in the current hold and retry early only when follower/enemy position, bearing, visibility, shootability, or under-fire state changes; cached per-frame cover evaluation prevents duplicate candidate passes.
