...
- exposed Rifleman `shootFromPlace` uses a narrow return-fire lease when a visible, shootable enemy beyond `8m` is looking at the follower: a follower that has not returned fire gets `0.55s`, one that has fired gets `0.9s`, and actual new damage can trigger recovery immediately. The static-fire action ends only after a concrete recovery movement is prepared; being in cover, point-blank contact, an enemy that is not looking at the follower, grenade-launcher fire, or a failed cover scan all preserve the current fire action.
