# SAINGrunt personalities and aggression

**Base contract:** [Core combat aggression](../../docs/Combat-Tactics.md) and [Core proficiency](../../docs/Friendly-AI-Performance-Settings.md). The addon consumes saved/temporary aggression to adapt tactical SAIN personality fields; it does not own mechanical difficulty. Native installation and restoration are addon-owned under the [integration contract](Integration.md). The display name is SAINGrunt; `SainMan` remains the persisted identifier.
...
The snapshot has no Git metadata, and byte identity with the installed SAIN DLL has not been established. Installed 4.5.1 API validation is separate; see [reference provenance](References.md). Do not treat source observations as measured raid behavior.
