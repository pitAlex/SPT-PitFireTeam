...
try {
    ...
    $firemode=$assembly.MainModule.Types | Where-Object FullName -eq 'SAIN.SAINComponent.Classes.WeaponFunction.Firemode'
    if(@($firemode.Methods | Where-Object {$_.Name -eq 'CheckSwapFireMode' -and $_.IsPublic -and !$_.IsStatic -and $_.ReturnType.FullName -eq 'System.Void' -and ($_.Parameters.ParameterType.FullName -join '|') -eq 'SAIN.Components.BotComponent|SAIN.SAINComponent.Classes.Info.BotWeaponInfoClass'}).Count -ne 1){throw 'Native fire-mode selection boundary changed'}
    ...
    if(@($self.Methods | Where-Object {$_.Name -eq 'startUseStims' -and !$_.IsStatic -and $_.ReturnType.FullName -eq 'System.Boolean' -and $_.Parameters.Count -eq 0}).Count -ne 1){throw 'Stim selection boundary changed'}
    if(@($self.Methods | Where-Object {$_.Name -eq 'ShallUseStimsCheckEnemy' -and $_.IsStatic -and $_.ReturnType.FullName -eq 'System.Boolean' -and ($_.Parameters.ParameterType.FullName -join '|') -eq 'SAIN.SAINComponent.Classes.EnemyClasses.Enemy'}).Count -ne 1){throw 'Stim safety boundary changed'}
...
try {
    ...
    $attention=$contactType.Methods | Where-Object Name -eq 'HandleAttentionCommand'
    $accepted=@($attention.Body.Instructions | Where-Object {$_.OpCode.Name -eq 'stfld' -and $_.Operand.Name -eq '_lastAttentionCommandAt'})
    $clear=@($attention.Body.Instructions | Where-Object {$_.Operand -is [Mono.Cecil.MethodReference] -and $_.Operand.Name -eq 'ClearEnemyStateForAttention'})
    if($accepted.Count -ne 1 -or $clear.Count -ne 1 -or $accepted[0].Offset -ge $clear[0].Offset){throw 'Core accepted Attention capture boundary changed'}
