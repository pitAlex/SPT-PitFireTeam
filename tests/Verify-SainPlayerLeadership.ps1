...
$source=Get-Content -Raw (Join-Path $RepositoryRoot 'client/Modules/SainPlayerSquadBridge.cs')
# Test stand-ins share this executable; production uses the real SAIN assembly.
$lookup='Type.GetType(name + ", SAIN", true)'
if (!$source.Contains($lookup)) { throw 'Review changed type resolution before updating this harness.' }
$source=$source.Replace($lookup,'LeadershipChecks.ResolveType(name)')
...
try {
    ...
    $arguments+=@(Join-Path $temporary 'Production.cs'; Join-Path $temporary 'Bridge.cs'; Join-Path $temporary 'Fixture.cs')
    & dotnet @arguments
